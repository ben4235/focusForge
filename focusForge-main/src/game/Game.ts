...
export class Game {
  ...
  applyUpgrade(fn:(s:GameState)=>void){
    // Apply the upgrade function to the current game state
    fn(this.state);
    // Clamp player stats to ensure upgrades cannot produce degenerate values
    const p = this.state.player;
    // Reload time should never drop too low or become excessively slow
    p.reload        = Math.max(0.18, Math.min(2.5, p.reload));
    // Attack speed (fire rate multiplier) has sensible bounds
    p.attackSpeed   = Math.max(0.5, Math.min(3.0, p.attackSpeed));
    // Convert to integer and limit number of projectiles per shot
    p.bulletsPerShot = Math.max(1, Math.min(8, (p.bulletsPerShot as any | 0)));
    // Spread cone should stay within 0–0.8 radians
    p.spreadRadians = Math.max(0, Math.min(0.8, p.spreadRadians));
  }
